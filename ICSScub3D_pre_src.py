...
print("*                                                                             *")
print("*     =================== Version 0.6 for Source Code ===================     *")
...
print("\nPlease wait...")
print("ICSScub3D is extracting magnetic shilding tensor from the output files...\n")
...
icssCubeFile.write(f" {int(sysAtomNumbers)}  {format(min(bqCoorsXList3), '.6f')}  {format(min(bqCoorsYList3), '.6f')}  {format(min(bqCoorsZList3), '.6f')}\n")
icssCubeFile.write(f" {len(bqCoorsXList3)}    {format(bqCoorsXList3[1] - bqCoorsXList3[0], '.6f')}    0.000000    0.000000\n")
icssCubeFile.write(f" {len(bqCoorsYList3)}    0.000000    {format(bqCoorsYList3[1] - bqCoorsYList3[0], '.6f')}    0.000000\n")
icssCubeFile.write(f" {len(bqCoorsZList3)}    0.000000    0.000000    {format(bqCoorsZList3[1] - bqCoorsZList3[0], '.6f')}\n")

for k in range(sysAtomNumbers):
	icssCubeFile.write(f" {elementNo(sysElemsList[k])}    {format(elementNo(sysElemsList[k]), '.6f')}    ")
	icssCubeFile.write(f" {format(float(sysCoorsXList[k]), '.6f')}    {format(float(sysCoorsYList[k]), '.6f')}    {format(float(sysCoorsZList[k]), '.6f')}\n")
